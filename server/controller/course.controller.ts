...
//import { generateVideoUrl } from './course.controller';
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (thumbnail && !thumbnail.startsWith("https")) {
        await cloudinary.v2.uploader.destroy(courseData.thumbnail.public_id); //courseData.
...
export const getCourseByUser = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!courseExists) {
        return next(
          new ErrorHandler("You are not eligible to acccess this course", 404)
        );
      }
  ...
  try {
   ...
   if(!courseContent){
    return next(new ErrorHandler("Invalid Content id",400))
      ...
      //add tis question to our course content
...
export const addReview = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      await redis.set(courseId as string, JSON.stringify(course), "EX", 604800); // 7days
